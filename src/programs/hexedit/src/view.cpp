...
static void hex_do_search(){
    if(!hex_prompt_buf[0]){ hex_set_status("Empty search", true); return; }
    uint32_t plen = pc_strlen(hex_prompt_buf);
    uint32_t start = hex_cursor + 1;
    if(start > hex_buf_len) start = 0;
    for(uint32_t i = start; i + plen <= hex_buf_len; ++i){
        bool mm = true;
        for(uint32_t j = 0; j < plen; ++j) if(hex_buf[i+j] != uint8_t(hex_prompt_buf[j])){ mm = false; break; }
        if(mm){ hex_cursor = i; hex_high_nibble = true; hex_ensure_visible(); hex_set_status("Found", false); return; }
    }
    for(uint32_t i = 0; i + plen <= hex_buf_len && i < start; ++i){
        bool mm = true;
        for(uint32_t j = 0; j < plen; ++j) if(hex_buf[i+j] != uint8_t(hex_prompt_buf[j])){ mm = false; break; }
        if(mm){ hex_cursor = i; hex_high_nibble = true; hex_ensure_visible(); hex_set_status("Found (wrapped)", false); return; }
    }
    hex_set_status("Not found", true);
}

void hex_exit_prompt(bool ok){
    if(!ok){ hex_prompt = HEXEDIT_PROMPT_NONE; hex_set_status("Cancelled", false); return; }
    if(hex_prompt == HEXEDIT_PROMPT_GOTO){
        bool good; uint32_t off = hex_parse_hex(hex_prompt_buf, &good);
        if(!good){ hex_set_status("Invalid hex", true); hex_prompt = HEXEDIT_PROMPT_NONE; return; }
        if(off > hex_buf_len) off = hex_buf_len;
        hex_cursor = off; hex_high_nibble = true; hex_ensure_visible(); hex_set_status("Goto done", false);
    } else if(hex_prompt == HEXEDIT_PROMPT_SEARCH) hex_do_search();
...
static void view_toolbar(){
    ...
    char info[64] = ""; char dec[12]; hex_u32_dec(dec, hex_buf_len); pc_copy(info, dec, sizeof(info)); hex_append_txt(info, " bytes", sizeof(info)); if(hex_dirty) hex_append_txt(info, " *", sizeof(info));
    pg_window_text(w, c.width - pc_strlen(info)*8 - 8, 8, info, hex_dirty ? 0xF9E2AF : w->theme.muted_text);
    const char* mode = hex_mode ? "HEX" : "ASCII"; const char* im = hex_insert_mode ? "INS" : "OVR"; char ms[20] = ""; pc_copy(ms, mode, sizeof(ms)); hex_append_txt(ms, "/", sizeof(ms)); hex_append_txt(ms, im, sizeof(ms));
...
static void view_status(){
    auto *w = &hex_window.gui; auto c = hex_window.client(); uint32_t y = c.height - HEXEDIT_STATUS_H;
    ...
    if(hex_prompt != HEXEDIT_PROMPT_NONE){
        ...
        uint32_t cx = lx + pc_strlen(hex_prompt_buf)*8; pg_window_rect(w, {cx, y+6, 8, 14}, 0x89B4FA);
        pg_window_text(w, c.width - 160, y + 8, "Enter OK Esc cancel", 0x7F849C);
        return;
    }
    char left[96] = "";
    if(hex_status_msg[0]){ pc_copy(left, hex_status_msg, sizeof(left)); pg_window_text(w, 8, y + 8, left, hex_status_err ? 0xF38BA8 : 0xF9E2AF); }
    else {
        char off[9]; hex_u32_hex(off, hex_cursor, 8); pc_copy(left, off, sizeof(left)); hex_append_txt(left, "  ", sizeof(left)); char dec[12]; hex_u32_dec(dec, hex_cursor); hex_append_txt(left, dec, sizeof(left));
        if(hex_cursor < hex_buf_len){ hex_append_txt(left, "  val 0x", sizeof(left)); char hb[3]; hb[0] = hex_digit(hex_buf[hex_cursor] >> 4); hb[1] = hex_digit(hex_buf[hex_cursor]); hb[2] = '\0'; hex_append_txt(left, hb, sizeof(left)); char a[8]; a[0]=' '; a[1]='\''; a[2]=(hex_buf[hex_cursor]>=32&&hex_buf[hex_cursor]<=126)?char(hex_buf[hex_cursor]):'.'; a[3]='\''; a[4]='\0'; hex_append_txt(left, a, sizeof(left)); }
    ...
    if(!hex_status_msg[0] && hw + 160 < c.width) pg_window_text(w, c.width - hw - 8, y + 8, help, 0x6C7086);
}

static void view_sidebar(){
    auto *w = &hex_window.gui; auto c = hex_window.client();
    ...
    if(hex_tree_visible_rows == 0) hex_tree_visible_rows = 1;
    ...
    for(uint32_t i = 0; i < hex_tree_visible_rows; ++i){
        ...
        if(int32_t(idx) >= hex_entry_count) break;
        ...
        if(hex_entries[idx].is_dir){ pg_window_rect(w, {10,y+3,12,8}, 0xF9E2AF); pg_window_rect(w, {10,y+8,14,6}, 0xF9E2AF); }
        else pg_window_rect(w, {10,y+2,10,12}, 0xCDD6F4);
        ...
        if(!hex_entries[idx].is_dir){
            char sz[16] = ""; uint64_t s = hex_entries[idx].size;
            if(s >= 1024){ char d[12]; hex_u32_dec(d, uint32_t(s/1024)); pc_copy(sz,d,sizeof(sz)); hex_append_txt(sz,"K",sizeof(sz)); }
            else { char d[12]; hex_u32_dec(d,uint32_t(s)); pc_copy(sz,d,sizeof(sz)); }
...
static void view_hex(uint32_t hex_x, uint32_t ascii_x){
    ...
    for(uint32_t r = 0; r < rows; ++r){
        ...
        if(r%2==1) pg_window_rect(w, {HEXEDIT_SIDEBAR_W,y,right_w,HEXEDIT_ROW_H}, 0x202131);
        char offs[9]; hex_u32_hex(offs, off, 8);
        ...
        for(uint32_t col = 0; col < HEXEDIT_BYTES_PER_ROW; ++col){
            ...
            if(abs < hex_buf_len){
                char hb[3]; hb[0]=hex_digit(hex_buf[abs]>>4); hb[1]=hex_digit(hex_buf[abs]); hb[2]='\0';
                ...
                if(cur){
                    ...
                    if(hex_mode && !hex_high_nibble) pg_window_rect(w,{x+8,y+12,8,2},0xF9E2AF);
        ...
        for(uint32_t col = 0; col < HEXEDIT_BYTES_PER_ROW; ++col){
            ...
            if(abs < hex_buf_len){
                ...
                else pg_window_text(w,x,y+4,s,(hex_buf[abs]>=32&&hex_buf[abs]<=126)?w->theme.text:w->theme.muted_text);
            } else if(abs==hex_buf_len && hex_cursor==hex_buf_len && col==(hex_buf_len%HEXEDIT_BYTES_PER_ROW)){
...
bool hex_do_draw(){
    if(hex_window.isMinimized()) return true;
    hex_window.begin();
    if(hex_window.isMinimized()){ hex_window.end(); return true; }
    auto c = hex_window.client();
