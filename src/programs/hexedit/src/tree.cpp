...
void hex_normalize_dir(char* out, uint32_t cap, const char* src){
    ...
    if(out[0] != '/'){
        char tmp[128]; pc_copy(tmp, out, sizeof(tmp));
        pc_copy(out, "/", cap);
        if(tmp[0]){
            uint32_t cur = pc_strlen(out);
            if(cur > 1) hex_append_txt(out, "/", cap);
...
void hex_refresh_dir(){
    ...
    for(int i = 0; i < hex_entry_count; ++i) for(int j = i + 1; j < hex_entry_count; ++j){
        bool si = hex_entries[i].is_dir, sj = hex_entries[j].is_dir;
        if(si != sj && sj){ HexDirEntry t = hex_entries[i]; hex_entries[i] = hex_entries[j]; hex_entries[j] = t; }
        else if(si == sj && pc_strcmp(hex_entries[i].name, hex_entries[j].name) > 0){ HexDirEntry t = hex_entries[i]; hex_entries[i] = hex_entries[j]; hex_entries[j] = t; }
    }
    hex_tree_scroll = 0;
}
