...
bool hex_reserve(uint32_t need){
    if(need <= hex_buf_cap) return true;
    ...
    if(!p) return false;
    if(hex_buf && p != (void*)(hex_buf + hex_buf_cap)) return false;
    if(!hex_buf) hex_buf = (uint8_t*)p;
    hex_buf_cap = cap;
    return true;
}
...
uint32_t hex_visible_rows(){
    ...
    if(c.height < HEXEDIT_TOOLBAR_H + HEXEDIT_HEADER_H + HEXEDIT_STATUS_H + HEXEDIT_ROW_H) return 1;
    return (c.height - HEXEDIT_TOOLBAR_H - HEXEDIT_HEADER_H - HEXEDIT_STATUS_H) / HEXEDIT_ROW_H;
}

uint32_t hex_total_rows(){
    if(hex_buf_len == 0) return 1;
...
void hex_clamp_view(){
    ...
    uint32_t max_off = 0;
    if(t > rows) max_off = (t - rows) * HEXEDIT_BYTES_PER_ROW;
    if(hex_view_offset > max_off) hex_view_offset = max_off;
...
void hex_ensure_visible(){
    ...
    if(cr < vr) hex_view_offset = cr * HEXEDIT_BYTES_PER_ROW;
    else if(cr >= vr + rows) hex_view_offset = (cr - rows + 1) * HEXEDIT_BYTES_PER_ROW;
    hex_clamp_view();
}
...
void hex_move_cursor(int32_t d){
    ...
    if(n < 0) n = 0;
    if(n > int64_t(hex_buf_len)) n = hex_buf_len;
...
bool hex_insert_byte(uint32_t off, uint8_t v){
    ...
    if(!hex_reserve(hex_buf_len + 1)) return false;
    for(uint32_t i = hex_buf_len; i > off; --i) hex_buf[i] = hex_buf[i-1];
    hex_buf[off] = v;
    hex_buf_len++;
    return true;
}
...
void hex_delete_byte(uint32_t off){
    if(off >= hex_buf_len) return;
    for(uint32_t i = off; i + 1 < hex_buf_len; ++i) hex_buf[i] = hex_buf[i+1];
    hex_buf_len--;
    if(hex_cursor > hex_buf_len) hex_cursor = hex_buf_len;
}
...
bool hex_load_file(const char* path){
    ...
    if(d < 0){
        hex_buf_len = 0; hex_cursor = 0; hex_view_offset = 0; hex_high_nibble = true; hex_dirty = false;
        pc_copy(hex_file_path, path, sizeof(hex_file_path));
        char tmp[128]; pc_copy(tmp, path, sizeof(tmp));
        int l = pc_strlen(tmp);
        for(int i = l - 1; i >= 0; --i) if(tmp[i] == '/'){ tmp[i] = '\0'; if(tmp[0] == '\0') pc_copy(tmp, "/", sizeof(tmp)); break; }
    ...
    hex_buf_len = 0; hex_cursor = 0; hex_view_offset = 0; hex_high_nibble = true; hex_dirty = false;
    if(!hex_reserve(4096)){ pc_file_close(d); return false; }
    for(;;){
        uint8_t chunk[512];
        int32_t c = pc_file_read(d, chunk, sizeof(chunk));
        if(c < 0){ pc_file_close(d); return false; }
        if(c == 0) break;
        if(!hex_reserve(hex_buf_len + uint32_t(c))){ pc_file_close(d); return false; }
        for(int i = 0; i < c; ++i) hex_buf[hex_buf_len++] = chunk[i];
    }
    pc_file_close(d);
    pc_copy(hex_file_path, path, sizeof(hex_file_path));
    char tmp[128]; pc_copy(tmp, path, sizeof(tmp));
    ...
    for(int i = 0; i < hex_entry_count; ++i) if(pc_strcmp(hex_entries[i].name, base) == 0) hex_sel_entry = i;
    hex_set_status("", false);
    return true;
}

bool hex_save_file(){
    if(!hex_file_path[0]) return false;
    if(pc_file_write(hex_file_path, hex_buf, hex_buf_len) < 0) return false;
    hex_dirty = false;
    return true;
}
