...
void settings_app_draw(struct settings_app *app, struct pg_window *w, const struct pg_event *ev){
    ...
    pg_window_text(w,14,16,"Settings",w->theme.text);
    pg_window_text(w,14,30,"PureC OS  •  /config/settings.ini",w->theme.muted_text);
    uint32_t sidebar_h=w->client.height-TOOLBAR_HEIGHT-STATUS_HEIGHT;
    ...
    pg_window_text(w,16,TOOLBAR_HEIGHT+132,"QUICK ACCESS",0x7F849C);
    pg_window_text(w,24,TOOLBAR_HEIGHT+156,"Volume / Mute",0x7F849C);
    pg_window_text(w,24,TOOLBAR_HEIGHT+174,"Output device",0x7F849C);
    ...
    pg_window_rect(w,(struct pg_rect){0,w->client.height-STATUS_HEIGHT,cw,STATUS_HEIGHT},0x292A3D);
    pg_window_text(w,12,w->client.height-20,app->tab==0?"Sound  •  changes saved instantly":"Wi-Fi  •  coming soon",0xBAC2DE);
    pg_window_end(w);
}
