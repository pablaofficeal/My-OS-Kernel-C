...
bool intel_ax201_init(void){
    ...
    for(uint32_t ci=0; ci < sizeof(candidates)/sizeof(candidates[0]); ci++){
        ...
        if(!ax201_get_firmware()){
            klogf(KLOG_DEBUG, "ax201: [BRUTE %u] %s NOT FOUND as Limine module, skip", ci+1, c->hint);
            continue;
        }
        klogf(KLOG_DEBUG, "ax201: [BRUTE %u] %s loaded %llu bytes, uploading...", ci+1, c->hint, (unsigned long long)adapter.firmware_size);
        if(!ax201_fw_upload()){
            klogf(KLOG_DEBUG, "ax201: [BRUTE %u] %s CTXT_INFO upload FAILED", ci+1, c->hint);
            continue;
        }
        klogf(KLOG_DEBUG, "ax201: [BRUTE %u] %s CTXT kick done, waiting ALIVE...", ci+1, c->hint);
        if(ax201_wait_alive_poll()){
            ...
            uint32_t hw_rev=ax201_csr_read(AX201_CSR_HW_REV);
            klogf(KLOG_DEBUG, "ax201: [BRUTE %u] %s ALIVE TIMEOUT gp=0x%08x int=0x%08x rev=0x%08x", ci+1, c->hint, gp, intr, hw_rev);
